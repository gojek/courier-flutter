...
NSString * const MQTTClientcourier = @"GJ";
...
@property (strong, nonatomic) GCDTimer *activityCheckTimer;
@property (strong, nonatomic) GCDTimer *connectCheckTimer;
...
- (instancetype)init {
    ...
    self.shouldEnableActivityCheckTimeout = false;
    self.activityCheckTimerInterval = 12;
...
- (void)decoder:(MQTTDecoder *)sender didReceiveMessage:(NSData *)data {
    ...
    @synchronized(sender) {
        self.lastInboundActivityTimestamp = [NSDate date].timeIntervalSince1970;
        DDLogVerbose(@"[RR] Log inbound activity: type: %hhu", message.type);
                        ...
                        if ((self.protocolLevel == MQTTProtocolVersion50 && message.data.length < 3) ||
                            ...
                            if (message.returnCode && (message.returnCode).intValue == MQTTSuccess) {
                                ...
                                if (self.effectiveKeepAlive > 0) {
                                    ...
                                    if (self.readTimeout < self.effectiveKeepAlive) {
                                        self.readTimeout = self.effectiveKeepAlive + 10;
                                    }
                                }
...
#pragma mark - Connect, Read, Inactivity Timeout

- (void)shouldLogForFastReconnectTimestampIfValid:(MQTTMessage *)message {
    if (self.shouldEnableActivityCheckTimeout) {
        switch (message.type) {
            case MQTTPublish:
...
- (void)checkAndSetFastReconnectTimestamp {
    DDLogVerbose(@"[RR] Outbound Fast Reconnect timestamp Check %@ <= last inbound activity timestamp %@", [NSDate dateWithTimeIntervalSince1970:self.fastReconnectTimestamp], [NSDate dateWithTimeIntervalSince1970:self.lastInboundActivityTimestamp]);
    if (self.fastReconnectTimestamp <= self.lastInboundActivityTimestamp) {
        self.fastReconnectTimestamp = [NSDate date].timeIntervalSince1970;
        DDLogVerbose(@"[RR] Outbound Set Fast Reconnect timestamp to %@", [NSDate dateWithTimeIntervalSince1970:self.fastReconnectTimestamp]);
    }
}

- (void)checkConnectActivity {
    DDLogVerbose(@"[RR] Check Connect Activity, connnect timestamp: %@, connect timeout: %f, current diff: %f", [NSDate dateWithTimeIntervalSince1970:self.connectTimestamp], self.connectTimeout, [NSDate date].timeIntervalSince1970 - self.connectTimestamp);

    if (self.status == MQTTSessionStatusConnecting) {
        if ([NSDate date].timeIntervalSince1970 - self.connectTimestamp >= self.connectTimeout) {
            NSError * error = [NSError errorWithDomain:MQTTSessionErrorDomain
                                                  code:MQTTSessionErrorConnectTimeout
                                              userInfo:@{NSLocalizedDescriptionKey : @"MQTT Session Connect Timeout"}];
            [self connectionError:error];
        }
    } else {
        if (self.connectCheckTimer) {
...
- (void)checkActivity {
    ...
    if (self.status == MQTTSessionStatusConnected) {
        if (self.fastReconnectTimestamp > self.lastInboundActivityTimestamp) {
            ...
            if ([NSDate date].timeIntervalSince1970 - self.fastReconnectTimestamp >= self.inactivityTimeout) {
                DDLogVerbose(@"[RR] Check Activity Disconnecting Inactivity timeout, Fast Reconnect: %@, last inbound activity: %@", [NSDate dateWithTimeIntervalSince1970:self.fastReconnectTimestamp], [NSDate dateWithTimeIntervalSince1970:self.lastInboundActivityTimestamp]);

                NSError * error = [NSError errorWithDomain:MQTTSessionErrorDomain
                                                      code:MQTTSessionErrorInactivityTimeout
                                                  userInfo:@{NSLocalizedDescriptionKey : @"MQTT Session Inactivity Timeout"}];
                [self connectionError:error];
            }
        } else if ([NSDate date].timeIntervalSince1970 - self.lastInboundActivityTimestamp > self.readTimeout) {
            DDLogVerbose(@"[RR] Check Activity Disconnecting Read Timeout, last inbound activity: %@, current diff: @%f, read timeout %f", [NSDate dateWithTimeIntervalSince1970:self.lastInboundActivityTimestamp], [NSDate date].timeIntervalSince1970 - self.lastInboundActivityTimestamp, self.readTimeout);

            NSError * error = [NSError errorWithDomain:MQTTSessionErrorDomain
                                                  code:MQTTSessionErrorReadTimeout
                                              userInfo:@{NSLocalizedDescriptionKey : @"MQTT Session Read Timeout"}];
            [self connectionError:error];
        }
    } else {
        if (self.activityCheckTimer) {
...
- (void)connect {
    ...
    self.connectTimestamp = [NSDate date].timeIntervalSince1970;
    __weak typeof(self) weakSelf = self;
    if (self.shouldEnableConnectCheckTimeout) {
        self.connectCheckTimer = [GCDTimer scheduledTimerWithTimeInterval:self.connectTimeoutCheckTimerInterval
                                                                   repeats:YES
                                                                     queue:self.queue
                                                                     block:^{ [weakSelf checkConnectActivity];
                                                                     }];
    }
...
- (BOOL)encode:(MQTTMessage *)message {
    if (message) {
        ...
        if (wireFormat) {
            ...
            DDLogVerbose(@"[RR] Log outbound activity: type - %hhu, qos - %hhu", message.type, message.qos);
            self.lastOutboundActivityTimestamp = [NSDate date].timeIntervalSince1970;
            [self shouldLogForFastReconnectTimestampIfValid:message];
...
- (void)mqttTransportDidClose:(id<MQTTTransport>)mqttTransport {
    ...
    dispatch_async(self.queue, ^{
        if (weakSelf != nil) {
            if (weakSelf.status == MQTTSessionStatusClosed || weakSelf.status == MQTTSessionStatusDisconnecting) {
                return;
            }
            [weakSelf error:MQTTSessionEventConnectionClosedByBroker error:nil];
        }
    });
}
